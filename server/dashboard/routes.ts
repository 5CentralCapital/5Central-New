...
export function registerDashboardRoutes(app: Express) {
  ...
  app.post("/api/plaid/sync", requireAdmin, async (_req, res) => {
    try {
      ...
      const { syncPlaidItem } = await import("./plaid-sync");
      const items = getPlaidItems();
      for (const item of items) {
        await syncPlaidItem(item, { client: plaidClient, upsertBankAccounts, upsertTransactions, removeTransactions, updatePlaidItemSync });
      }
      res.json({ ok: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
