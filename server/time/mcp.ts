...
export function registerTimeMcpTools(register: TimeToolRegistrar, options: TimeMcpOptions): void {
  ...
  register("list_time_entries", "List scoped QuickBooks Time entries for admin review. Provider clock-in data, R-ops approval, estimated labor and posted payroll remain separate states.", { query: timeListQuerySchema }, false,
  ...
  register("list_time_jobcodes", "List provider jobcodes available for R-ops project and property mapping.", { scope: timeConnectionScopeSchema }, false,
  ...
  register("sync_time_records", "Mirror provider users, jobcodes, timesheets and deletion tombstones into R-ops. This does not write back to QuickBooks Time or post payroll.", { scope: timeConnectionScopeSchema, maxPages: z.number().int().min(1).max(10_000).optional() }, true,
  ...
  for (const kind of TIME_COMMAND_KINDS) {
    register(kind.replaceAll(".", "_"), `Save ${kind.replaceAll(".", " ")} in R-ops. Supply a stable operationId/idempotencyKey and retry an uncertain response with the identical envelope. This does not alter QuickBooks Time or post payroll.`, { command: commandEnvelopeSchema(timeCommandPayloadSchemas[kind]) }, true,
      async ({ command }) => {
