...
The Accounting client supports read-by-ID, QBO query reads, create, and update. Updates require the caller to provide the current `Id` and `SyncToken`; the adapter sends both in the update payload. Stale-object fault 5010 is a `quickbooks_conflict` and is never retried. Every create and update carries Intuit's `requestid` query parameter; the write reconciler derives it deterministically from the local operation key so any retry of the same logical write is de-duplicated by Intuit. GET failures expose a safe retry classification. POST failures from timeouts, transport failures, 408, 429, 5xx, or an unparseable success response are `quickbooks_ambiguous_write` and must be reconciled by provider read-back before another write is attempted. A 429 starts a per-realm cooldown of at least 60 seconds. The adapter performs no automatic retries.

Token refresh is serialized across workers by the database refresh lease. A worker that loses the lease waits a bounded time for the winner's committed token, and an `invalid_grant` is treated as a reconnect requirement only when the rejected refresh token is still the latest stored one. Disconnect holds the same lease and wipes only the exact credential version it revoked. Intuit issues a new refresh-token value about once every 24 hours, so a refresh inside that window legitimately returns the same value.

Provider sync (`server/accounting/provider-sync.ts`) mirrors Purchase, Bill, BillPayment, Deposit and Account. A provider object that cannot be normalized exactly is never mirrored partially: its revision is kept for audit, any older mirrored lines are retired, and a durable row is written to `accounting_qbo_sync_exceptions`. Coverage stays `partial` until a later revision of that object mirrors completely, so an advancing checkpoint or a later incremental run cannot hide a gap. Incremental runs are always `partial` because QBO queries do not return deletions; `catchUp({ fullReplay: true })` re-reads every object, reconciles, and flags mirrored objects that QBO no longer returns. Pagination uses a LastUpdatedTime keyset cursor so an edit during a sync cannot shift unseen records behind the watermark. See [the requirements matrix](../../../docs/company/qbo-requirements-matrix.md).
...
Webhook verification must receive the exact raw request bytes before JSON parsing. `verifyQuickBooksWebhookSignature` checks the base64 HMAC-SHA256 digest against the `intuit-signature` header in constant time. `parseQuickBooksWebhookPayload` accepts only Intuit's CloudEvents array format (the legacy `eventNotifications` envelope was retired on 2026-07-31). No webhook route is registered yet; one needs a durable event ledger, a 3-second acknowledgment with asynchronous processing, and a separate verifier token per environment.
