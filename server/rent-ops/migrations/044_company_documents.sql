-- Company documents with immutable verified sources, record links and staged uploads.
-- Promoted from server/company-documents/schema.sql.
SELECT 1 / CASE WHEN EXISTS (
  SELECT 1 FROM rent_ops_schema_migrations
  WHERE version = 43 AND checksum_sha256 = '__RENT_OPS_V43_CHECKSUM__'
) THEN 1 ELSE 0 END AS company_documents_predecessor_guard;
...
VALUES (44, '__RENT_OPS_V44_CHECKSUM__')
...
SELECT 1 / CASE WHEN EXISTS (
  SELECT 1 FROM rent_ops_schema_migrations WHERE version = 44 AND checksum_sha256 = '__RENT_OPS_V44_CHECKSUM__'
) THEN 1 ELSE 0 END AS company_documents_checksum_guard;
