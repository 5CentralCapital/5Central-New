...
function descriptors(snapshot: RentOpsSnapshot, financialSemanticCrosswalk?: RentManagerFinancialSemanticCrosswalk): CollectionDescriptor[] {
  ...
  const collections: CollectionDescriptor[] = [
    ...
    { name: "people", table: "rent_ops_people", records: snapshot.people, columns: ["id", "payment_review_reason", "payment_review_artifact_sha256", "payment_review_source_reference", "first_name", "last_name", "email", "phone", "phone_methods", "first_name_knowledge", "last_name_knowledge", "email_knowledge", "phone_knowledge", "renter_insurance_expires_on", "archived", "archived_knowledge", "source_system", "source_id"], values: (record) => { const row = record as RentOpsPerson; return [row.id, nullable(row.paymentReviewReason), nullable(row.paymentReviewArtifactSha256), nullable(row.paymentReviewSourceReference), nullable(row.firstName), nullable(row.lastName), nullable(row.email), nullable(row.phone), jsonValue(row.phoneMethods), nullable(row.firstNameKnowledge), nullable(row.lastNameKnowledge), nullable(row.emailKnowledge), nullable(row.phoneKnowledge), nullable(row.renterInsuranceExpiresOn), nullable(row.archived), nullable(row.archivedKnowledge), ...source(row)]; } },
