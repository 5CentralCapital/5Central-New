...
function totalsFor(snapshot: RentOpsSnapshot): PersistenceImportTotals {
  ...
  return {
    ...
    depositsCents: sum(snapshot.securityDeposits.map((row) => row.amountHeldCents)),
...
function moneyStateCounts(snapshot: RentOpsSnapshot, exceptions: readonly ImportMappingException[]): { unknown: Record<string, number>; invalid: Record<string, number> } {
  ...
  const classifyRows = (key: string, rows: readonly { amountCents?: number | null; source?: { sourceId?: string } }[], unknownCode: string, invalidCode: string) => {
    const unknownSources = exceptionSources(unknownCode);
    const invalidSources = exceptionSources(invalidCode);
...
function validateOrphans(snapshot: RentOpsSnapshot): string[] {
  ...
  for (const row of snapshot.tenancies) if (!(v3 && (!row.propertyId || !row.unitId || !row.primaryPersonId) && [row.propertyId ? undefined : row.propertyLinkKnowledge, row.unitId ? undefined : row.unitLinkKnowledge, row.primaryPersonId ? undefined : row.primaryPersonLinkKnowledge].every((knowledge) => knowledge === "unknown" || knowledge === "ambiguous")) && (!row.propertyId || !row.unitId || !row.primaryPersonId || !propertyIds.has(row.propertyId) || unitMap.get(row.unitId)?.propertyId !== row.propertyId || !peopleIds.has(row.primaryPersonId))) reasons.push("orphan_tenancy");
