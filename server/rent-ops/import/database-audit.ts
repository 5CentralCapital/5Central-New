...
export const DATABASE_AUDIT_SQL = Object.freeze({
  ...
  orphans: `
    SELECT
      ...
      (SELECT COUNT(*) FROM rent_ops_payment_allocations a LEFT JOIN rent_ops_ledger_transactions p ON p.id = CASE WHEN a.kind='credit_allocation' THEN a.credit_transaction_id ELSE a.payment_transaction_id END WHERE p.id IS NULL) AS allocations_payment,
  ...
  allocationInvariants: `
    SELECT
      ...
      (SELECT COUNT(*) FROM rent_ops_payment_allocations a LEFT JOIN rent_ops_ledger_transactions c ON c.id=a.credit_transaction_id LEFT JOIN rent_ops_ledger_transactions q ON q.id=a.charge_transaction_id WHERE a.kind='credit_allocation' AND (c.id IS NULL OR c.kind IS DISTINCT FROM 'credit' OR a.payment_transaction_id IS NOT NULL OR a.credit_link_knowledge IS DISTINCT FROM 'exact' OR (c.status IS NOT NULL AND c.status<>'posted') OR (c.property_id IS NOT NULL AND q.property_id IS NOT NULL AND c.property_id<>q.property_id AND NOT COALESCE((a.source_system='rent_manager' AND a.source_artifact_sha256=c.source_artifact_sha256 AND a.source_artifact_sha256=q.source_artifact_sha256 AND c.person_id=q.person_id AND c.person_link_knowledge='exact' AND q.person_link_knowledge='exact' AND a.source_property_id IN (c.property_id,q.property_id)),false)) OR (c.tenancy_id IS NOT NULL AND q.tenancy_id IS NOT NULL AND c.tenancy_id<>q.tenancy_id) OR (c.person_id IS NOT NULL AND q.person_id IS NOT NULL AND c.person_id<>q.person_id) OR (a.source_property_id IS NOT NULL AND a.source_property_id IS DISTINCT FROM c.property_id AND a.source_property_id IS DISTINCT FROM q.property_id) OR (a.allocated_on<q.posted_on AND NOT COALESCE((a.source_system='rent_manager' AND a.source_artifact_sha256=c.source_artifact_sha256 AND a.source_artifact_sha256=q.source_artifact_sha256 AND c.person_id=q.person_id AND c.person_link_knowledge='exact' AND q.person_link_knowledge='exact' AND a.source_property_id IN(c.property_id,q.property_id) AND q.source_updated_at IS NOT NULL AND (q.source_updated_at AT TIME ZONE 'UTC')::date<=a.allocated_on),false)))) AS credit_allocation_parent_invalid,
      (SELECT COUNT(*) FROM (SELECT source_row.*, SUM(amount_cents) OVER (PARTITION BY payment_transaction_id,charge_transaction_id ORDER BY allocated_on,amount_cents DESC,id ROWS UNBOUNDED PRECEDING) AS running_amount FROM rent_ops_payment_allocations source_row WHERE kind <> 'transfer') a WHERE running_amount < 0 AND NOT COALESCE((a.kind='reversal' AND a.source_system='rent_manager' AND a.source_artifact_sha256 ~ '^[a-f0-9]{64}$' AND a.payment_link_knowledge='exact' AND a.charge_link_knowledge='exact' AND EXISTS(SELECT 1 FROM rent_ops_ledger_transactions r WHERE r.kind='reversal' AND r.status='posted' AND r.reversal_of_id=a.payment_transaction_id AND r.posted_on=a.allocated_on) AND (SELECT COALESCE(SUM(f.amount_cents),0) FROM rent_ops_payment_allocations f WHERE f.kind='allocation' AND f.source_system='rent_manager' AND f.source_artifact_sha256=a.source_artifact_sha256 AND f.payment_transaction_id=a.payment_transaction_id AND f.charge_transaction_id=a.charge_transaction_id AND f.payment_link_knowledge='exact' AND f.charge_link_knowledge='exact' AND f.amount_knowledge='known' AND f.amount_cents>0 AND f.allocated_on>a.allocated_on AND (f.source_updated_at AT TIME ZONE 'UTC')::date<a.allocated_on)>=-a.running_amount),false)) AS allocation_reversal_exceeds_history
  `,
  /**
  ...
  dates: `
    SELECT
      ...
      (SELECT COUNT(*) FROM rent_ops_payment_allocations a JOIN rent_ops_ledger_transactions c ON c.id = a.charge_transaction_id WHERE a.allocated_on < c.posted_on AND NOT COALESCE((
        (a.kind='credit_allocation' AND a.source_system='rent_manager' AND a.source_artifact_sha256 ~ '^[a-f0-9]{64}$' AND a.credit_link_knowledge='exact' AND a.charge_link_knowledge='exact' AND a.source_artifact_sha256=c.source_artifact_sha256 AND c.source_updated_at IS NOT NULL AND (c.source_updated_at AT TIME ZONE 'UTC')::date<=a.allocated_on AND EXISTS(SELECT 1 FROM rent_ops_ledger_transactions cr WHERE cr.id=a.credit_transaction_id AND cr.kind='credit' AND cr.source_system='rent_manager' AND cr.source_artifact_sha256=a.source_artifact_sha256 AND cr.person_id=c.person_id AND cr.person_link_knowledge='exact' AND c.person_link_knowledge='exact' AND a.source_property_id IN(cr.property_id,c.property_id)))
        OR (a.kind='reversal' AND a.source_system='rent_manager' AND a.source_artifact_sha256 ~ '^[a-f0-9]{64}$' AND a.amount_cents<0 AND a.payment_link_knowledge='exact' AND a.charge_link_knowledge='exact' AND EXISTS(SELECT 1 FROM rent_ops_ledger_transactions r WHERE r.kind='reversal' AND r.status='posted' AND r.reversal_of_id=a.payment_transaction_id AND r.posted_on=a.allocated_on) AND (SELECT COALESCE(SUM(f.amount_cents),0) FROM rent_ops_payment_allocations f WHERE f.kind='allocation' AND f.source_system='rent_manager' AND f.source_artifact_sha256=a.source_artifact_sha256 AND f.payment_transaction_id=a.payment_transaction_id AND f.charge_transaction_id=a.charge_transaction_id AND f.payment_link_knowledge='exact' AND f.charge_link_knowledge='exact' AND f.amount_knowledge='known' AND f.amount_cents>0 AND f.allocated_on>a.allocated_on AND (f.source_updated_at AT TIME ZONE 'UTC')::date<a.allocated_on)>=-a.amount_cents)
      ),false)) AS allocation_before_charge,
  ...
  propertyControls: `
    SELECT
      ...
      (SELECT COALESCE(SUM(a.amount_cents), 0)::bigint FROM rent_ops_payment_allocations a LEFT JOIN rent_ops_ledger_transactions pay ON pay.id = a.payment_transaction_id JOIN rent_ops_ledger_transactions charge ON charge.id = a.charge_transaction_id WHERE pay.property_id = p.id OR charge.property_id = p.id) AS allocations_cents,
