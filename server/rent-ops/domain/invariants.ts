...
export function validateSnapshot(snapshot: RentOpsSnapshot): InvariantViolation[] {
  ...
  for (const allocation of snapshot.paymentAllocations) {
    ...
    if (allocation.kind !== "transfer" && !reversedAllocationTargets.has(allocation.chargeTransactionId??"") && allocationParentId && typeof allocation.amountCents === "number") allocationsByPayment.set(allocationParentId, (allocationsByPayment.get(allocationParentId) ?? 0) + allocation.amountCents);
