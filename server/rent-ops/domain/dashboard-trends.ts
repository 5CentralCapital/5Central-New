...
export function deriveDashboardTrends(snapshot: RentOpsSnapshot, filters: RentOpsFilters = {}): DashboardTrends {
  ...
  const archivedSnapshots = deriveArchivedDashboardSnapshots(snapshot, filters, asOfDate).snapshots;
  return dashboardTrendsSchema.parse({
    ...
    // Keep the legacy response shape when no validated archive exists.
    ...(archivedSnapshots.length ? { archivedSnapshots } : {}),
  });
}
