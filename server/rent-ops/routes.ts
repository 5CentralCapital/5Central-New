...
import { correctChargeSchema, correctPaymentSchema, manualPaymentSchema, createChargeDefinitionSchema, patchChargeDefinitionSchema } from "./services/operational-inputs";
...
export function createRentOpsRouter(options: RentOpsRouteOptions): Router {
  ...
  adminRouter.get("/charges/:id/edit",async(req,res)=>{try{const result=await service.chargeEditContext(req.params.id);res.json({charge:serializeAdminLedgerTransaction(result.charge),expectedRevision:result.expectedRevision,appliedCents:result.allocations.reduce((sum,row)=>sum+(row.amountCents??0),0)});}catch(error){res.status(409).json({error:error instanceof RentOpsInvariantError?error.message:"Charge could not be loaded"});}});
  adminRouter.post("/charges/:id/corrections",async(req,res)=>{
    const parsed=correctChargeSchema.safeParse(req.body);if(!parsed.success){res.status(400).json(errorBody("invalid_input"));return;}
    const actorSubject=req.rentOpsAdminUser?.id;if(!actorSubject){res.status(401).json(errorBody("not_authorized"));return;}
    try{const result=await service.correctCharge(req.params.id,parsed.data,{actorSubject,occurredAt:patchOccurredAt()});res.status(result.replayed?200:201).json({charge:serializeAdminLedgerTransaction(result.charge),replayed:result.replayed});}catch(error){if(error instanceof RentOpsInvariantError)res.status(409).json({error:error.message});else adminError(res,error);}
  });
