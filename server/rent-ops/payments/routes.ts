...
function failure(res:Response,error:unknown) {const known=error instanceof TenantPaymentError;res.status(known?error.status:503).json({error:known?error.code:'payment_service_unavailable'});}
