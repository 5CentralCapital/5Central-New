...
test("shared account service keeps grants email-free and blocked recovery preserves credentials",async()=>{
 ...
 await assert.rejects(service.grant({email:"resident@example.test",personId:"wrong",tenancyId:"demo-tenancy-1"},context));
 assert.equal((await store.list()).length,0);
 await assert.rejects(service.grant({email:"resident@example.test",personId:"demo-person-1",tenancyId:"demo-tenancy-1"},{actorSubject:""}),/Verified administrator identity/);
 assert.equal((await store.list()).length,0);
 const granted=await service.grant({email:"resident@example.test",personId:"demo-person-1",tenancyId:"demo-tenancy-1"},context);
 ...
 await service.revoke(active.id,active.sessionVersion,context);
 assert.equal(store.audits.length,2);
 assert.deepEqual(store.audits.map(({action,actorSubject})=>({action,actorSubject})),[
  {action:"grant",actorSubject:"verified-manager"},{action:"revoke",actorSubject:"verified-manager"},
 ]);
 assert.doesNotMatch(JSON.stringify(store.audits),/token|password|secret|working-password-hash/);
