...
export interface TenantAccountAdminContext { actorSubject: string; }
export function tenantAccountSummary(record: TenantAccountRecord): TenantAccountSummary {
  return { id:record.id,email:record.email,personId:record.personId,tenancyId:record.tenancyId,status:record.status,credentialRevision:record.sessionVersion,createdAt:record.createdAt,activatedAt:record.activatedAt,invitationExpiresAt:record.invitationExpiresAt };
}
const requestIdSchema = z.string().min(8).max(160).regex(/^[A-Za-z0-9_-]+$/);
const credentialRevisionSchema = z.number().int().positive().max(2147483646);
const grantSchema = z.object({email:z.string().trim().email().max(240).transform(value=>value.toLowerCase()),personId:z.string().min(1).max(160),tenancyId:z.string().min(1).max(160),requestId:requestIdSchema.optional()}).strict();
...
export class TenantAccountAdminService {
  ...
  private async mutateForMcp(action:"reissue"|"revoke",id:string,expectedCredentialRevision:number,context:{actorSubject:string}) {
    const actorSubject=this.actor(context);
    credentialRevisionSchema.parse(expectedCredentialRevision);
    const current=action==="reissue"?await this.bound(id):await this.options.store.getById(id);
    if(!current) throw new TenantPortalError(404,"Tenant account was not found.");
    const token=this.token(24*60*60*1000);
  ...
  async grant(input:unknown, context:TenantAccountAdminContext) {
    const actorSubject=this.actor(context);
    const parsed=grantSchema.parse(input);
    const requestId=parsed.requestId ?? randomUUID();
    const {requestId:_requestId,...value}=parsed;
    if (!eligibleTenantTenancies(await this.options.repository.getSnapshot()).some(row=>row.personId===value.personId&&row.tenancyId===value.tenancyId)) throw new TenantPortalError(400,"Select an exact current or future primary tenant before creating an account.");
    const id=`tenant-account-${createHash("sha256").update(actorSubject+":"+requestId).digest("hex")}`;
    const existing=await this.options.store.getById(id);
    if(existing) {
      if(existing.email!==value.email||existing.personId!==value.personId||existing.tenancyId!==value.tenancyId) throw new TenantPortalError(409,"Request identifier already belongs to another account grant.");
      throw new TenantPortalError(409,"This account grant already completed. Refresh the account and create a new access link if needed.");
    }
    const token=this.token(24*60*60*1000);
    const account=await this.options.store.auditedMutation!({action:"grant",id,...value,actorSubject,tokenHash:token.hash,expiresAt:token.expiresAt,now:token.now});
  ...
  async reissue(id:string, expectedCredentialRevision:number, context:TenantAccountAdminContext) {
    const actorSubject=this.actor(context);
    credentialRevisionSchema.parse(expectedCredentialRevision);
    const current=await this.bound(id);
    if(current.sessionVersion!==expectedCredentialRevision) throw new TenantPortalError(409,"Account access changed. Refresh and try again.");
    const token=this.token(24*60*60*1000);
    const account=await this.options.store.auditedMutation!({action:"reissue",id,actorSubject,personId:current.personId,tenancyId:current.tenancyId,expectedCredentialRevision,tokenHash:token.hash,expiresAt:token.expiresAt,now:token.now});
    if(!account) throw new TenantPortalError(409,"Account access changed. Refresh and try again.");
    return {account:tenantAccountSummary(account),activationPath:`/tenant#activate=${token.token}`,expiresAt:token.expiresAt};
  }
  async revoke(id:string, expectedCredentialRevision:number, context:TenantAccountAdminContext) {
    const actorSubject=this.actor(context);
    credentialRevisionSchema.parse(expectedCredentialRevision);
    const current=await this.options.store.getById(id);
    if(!current) throw new TenantPortalError(404,"Tenant account was not found.");
    if(current.sessionVersion!==expectedCredentialRevision) throw new TenantPortalError(409,"Account access changed. Refresh and try again.");
    const account=await this.options.store.auditedMutation!({action:"revoke",id,actorSubject,personId:current.personId,tenancyId:current.tenancyId,expectedCredentialRevision,now:this.now().toISOString()});
    if(!account) throw new TenantPortalError(409,"Account access changed. Refresh and try again.");
