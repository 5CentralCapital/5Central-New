...
The version 2 financial metadata derivative translates exact Rent Manager `charges/Charge`, `payments/Payment`, and `credits/Credit` endpoint records to canonical `posted`: recorded in the account ledger. Its plan binds captured primary documentation and the source artifact. This does not establish bank clearance, settlement, current due status, or the resident's share of assisted rent. Explicit contradictory status/void evidence remains blocked. Five source-bound assistance review flags preserve unresolved payment responsibility without creating subsidy contracts or amounts. Recurring schedules remain unknown and nonbillable until their source activation, frequency, and effective-date meanings are verified.
