...
const patchColumns: Record<RentOpsPatchEntityType, ReadonlySet<string>> = {
  ...
  security_deposit: new Set(["property_id", "unit_id", "tenancy_id", "person_id", "type", "amount_held_cents", "received_on", "disposition_status", "disposed_on", "disposition_notes", "property_link_knowledge", "unit_link_knowledge", "person_link_knowledge", "type_knowledge", "received_on_knowledge", "disposition_status_knowledge"]),
...
export class PostgresRentOpsRepository implements RentOpsRepository {
  ...
  async savePaymentAllocation(value: RentOpsPaymentAllocation): Promise<RentOpsPaymentAllocation> { assertImportedAllocation(value); await this.insertOnly("rent_ops_payment_allocations", ["id", "payment_transaction_id", "charge_transaction_id", "amount_cents", "allocated_on", "payment_link_knowledge", "charge_link_knowledge", "amount_knowledge", "allocated_on_knowledge", "source_system", "source_id"], [value.id, value.paymentTransactionId, value.chargeTransactionId, value.amountCents, value.allocatedOn, value.paymentLinkKnowledge, value.chargeLinkKnowledge, value.amountKnowledge, value.allocatedOnKnowledge, value.source?.system, value.source?.sourceId]); return value; }
  async saveSecurityDeposit(value: RentOpsSecurityDeposit): Promise<RentOpsSecurityDeposit> { assertPositiveCents(value.amountHeldCents, "deposit amountHeldCents"); await this.upsert("rent_ops_security_deposits", ["id", "property_id", "property_link_knowledge", "unit_id", "unit_link_knowledge", "tenancy_id", "person_id", "person_link_knowledge", "type", "type_knowledge", "amount_held_cents", "received_on", "received_on_knowledge", "disposition_status", "disposition_status_knowledge", "disposed_on", "disposition_notes", "source_system", "source_id"], [value.id, value.propertyId, value.propertyLinkKnowledge, value.unitId, value.unitLinkKnowledge, value.tenancyId, value.personId, value.personLinkKnowledge, value.type, value.typeKnowledge, value.amountHeldCents, value.receivedOn, value.receivedOnKnowledge, value.dispositionStatus, value.dispositionStatusKnowledge, value.disposedOn, value.dispositionNotes, value.source?.system, value.source?.sourceId]); return value; }
