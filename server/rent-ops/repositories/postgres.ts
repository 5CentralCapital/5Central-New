...
const patchColumns: Record<RentOpsPatchEntityType, ReadonlySet<string>> = {
  ...
  unit: new Set(["vacancy_confirmed_on", "vacancy_confirmation_knowledge", "property_id", "unit_number", "unit_type", "bedrooms", "bathrooms", "square_feet", "market_rent_cents", "default_deposit_cents", "readiness", "listing", "amenities", "access_notes", "property_link_knowledge", "unit_number_knowledge", "unit_type_knowledge", "readiness_knowledge", "listing_knowledge"]),
  person: new Set(["source_account_facts", "first_name", "last_name", "email", "phone", "phone_methods", "renter_insurance_expires_on", "archived", "first_name_knowledge", "last_name_knowledge", "email_knowledge", "phone_knowledge", "archived_knowledge"]),
  ...
  application: new Set(["converted_tenancy_id", "updated_at", "updated_at_knowledge", "status", "email", "first_name", "last_name", "phone", "property_id", "unit_id", "submitted_on", "certification_accepted_on", "rental_history", "employment", "household_summary", "preferences", "voucher", "pets", "vehicles", "emergency_contact", "profile_answers", "status_knowledge", "email_knowledge", "first_name_knowledge", "last_name_knowledge", "phone_knowledge", "property_link_knowledge", "unit_link_knowledge", "submitted_on_knowledge", "certification_accepted_on_knowledge"]),
...
function rowToAllocation(row: Record<string, unknown>): RentOpsPaymentAllocation {
  return {
    ...
    sourceArtifactSha256: nullableTextValue(row, "sourceArtifactSha256", "source_artifact_sha256"),
    artifactObservationOn: nullableDateValue(get(row, "artifactObservationOn", "artifact_observation_on")),
    sourcePropertyId: nullableTextValue(row,"sourcePropertyId","source_property_id"),
...
export class PostgresRentOpsRepository implements RentOpsRepository {
  ...
  async readPortalAccountBindings(personId: string) {
    return readPortalAccountBindings(this.client, personId);
  }

  async transferPortalAccountBinding(input: RentOpsPortalAccountTransfer) {
    if (!this.inTransaction) throw new RentOpsInvariantError("Portal account transfer requires the enclosing reconciliation transaction");
    return transferPortalAccountBinding(this.client, input);
  }
  ...
  async savePerson(value: RentOpsPerson): Promise<RentOpsPerson> { await this.upsert("rent_ops_people", ["id", "source_account_facts", "payment_review_reason", "payment_review_artifact_sha256", "payment_review_source_reference", "first_name", "last_name", "email", "phone", "phone_methods", "first_name_knowledge", "last_name_knowledge", "email_knowledge", "phone_knowledge", "renter_insurance_expires_on", "archived", "archived_knowledge", "source_system", "source_id"], [value.id, value.sourceAccountFacts ? JSON.stringify(value.sourceAccountFacts) : null, value.paymentReviewReason, value.paymentReviewArtifactSha256, value.paymentReviewSourceReference, value.firstName, value.lastName, value.email, value.phone, value.phoneMethods ? JSON.stringify(value.phoneMethods) : null, value.firstNameKnowledge, value.lastNameKnowledge, value.emailKnowledge, value.phoneKnowledge, value.renterInsuranceExpiresOn, value.archived, value.archivedKnowledge, value.source?.system, value.source?.sourceId]); return value; }
  ...
  async saveLedgerTransaction(value: RentOpsLedgerTransaction): Promise<RentOpsLedgerTransaction> {
    ...
    await this.insertOnly(
      ...
      [
        ...
        value.sourceArtifactSha256 ?? null, value.artifactObservationOn ?? null, value.source?.system ?? null, value.source?.sourceId ?? null, value.source?.sourceUpdatedAt ?? null,
      ],
    );
    return value;
  }
  async savePaymentAllocation(value: RentOpsPaymentAllocation): Promise<RentOpsPaymentAllocation> { assertImportedAllocation(value); await this.insertOnly("rent_ops_payment_allocations", ["id", "payment_transaction_id", "charge_transaction_id", "amount_cents", "allocated_on", "payment_link_knowledge", "charge_link_knowledge", "amount_knowledge", "allocated_on_knowledge", "source_system", "source_id", "kind", "source_artifact_sha256", "artifact_observation_on", "source_updated_at", "credit_transaction_id", "credit_link_knowledge", "source_property_id"], [value.id, value.paymentTransactionId, value.chargeTransactionId, value.amountCents, value.allocatedOn, value.paymentLinkKnowledge, value.chargeLinkKnowledge, value.amountKnowledge, value.allocatedOnKnowledge, value.source?.system, value.source?.sourceId, value.kind ?? "allocation", value.sourceArtifactSha256 ?? null, value.artifactObservationOn ?? null, value.source?.sourceUpdatedAt ?? null, value.creditTransactionId ?? null, value.creditLinkKnowledge ?? null, value.sourcePropertyId ?? null]); return value; }
  ...
  async getDocumentObjectBinding(documentId: string): Promise<RentOpsDocumentObjectBinding | undefined> {
    ...
    if (!bindingDocumentId || !backend || !logicalKey || !checksumSha256 || sizeBytes === undefined || !verifiedAt || (bindingKind !== "applicant" && bindingKind !== "import" && bindingKind !== "admin")) throw new RentOpsInvariantError("Verified document binding is incomplete");
