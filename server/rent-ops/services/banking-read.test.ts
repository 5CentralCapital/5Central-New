...
test("banking is production-only, strips raw fields, keeps bank signs and nullable money", async () => {
 ...
 assert.ok(!/SECRET|PRIVATE_ACCOUNT|PRIVATE_TX/.test(JSON.stringify(data))); assert.equal(calls.length, 3);
});
...
test("transaction pagination is bounded, validates amounts, and labels incomplete results", async () => {
 let txCalls = 0;
 const data = await readBanking(env, (async url => { if (String(url).endsWith("item/get")) return new Response("{}"); if (String(url).endsWith("balance/get")) return new Response(JSON.stringify({ accounts: [account] })); txCalls++; return new Response(JSON.stringify({ accounts: [account], transactions: [{ ...transaction, transaction_id: `tx-${txCalls}`, amount: 1e100 }], total_transactions: 100 })); }) as typeof fetch, now);
...
test("item freshness is bank update time, separately sanitized from fetch time", async () => {
 const data = await readBanking(env, (async url => new Response(JSON.stringify(String(url).endsWith("item/get") ? { status: { transactions: { last_successful_update: "2026-09-09T14:00:00Z", last_failed_update: "2026-09-10T14:00:00Z" } }, request_id: "SECRET_REQUEST" } : String(url).endsWith("balance/get") ? { accounts: [account] } : { accounts: [account], transactions: [transaction], total_transactions: 1 }))) as typeof fetch, now);
 assert.equal(data.connections[0].transactionsLastSuccessfulUpdate, "2026-09-09T14:00:00.000Z");
 assert.equal(data.connections[0].transactionsLastFailedUpdate, "2026-09-10T14:00:00.000Z");
 assert.notEqual(data.fetchedAt, data.connections[0].transactionsLastSuccessfulUpdate);
 assert.ok(bankingSnapshotSchema.safeParse(data).success); assert.ok(!JSON.stringify(data).includes("SECRET_REQUEST"));
});
test("freshness outage or malformed timestamps never erase balances or transactions", async () => {
 for (const mode of ["failure", "invalid"] as const) {
  const data = await readBanking(env, (async url => {
   if (String(url).endsWith("item/get")) return mode === "failure" ? new Response("SECRET_UPSTREAM", { status: 500 }) : new Response(JSON.stringify({ status: { transactions: { last_successful_update: "SECRET_INVALID", last_failed_update: null } } }));
   return new Response(JSON.stringify(String(url).endsWith("balance/get") ? { accounts: [account] } : { accounts: [account], transactions: [transaction], total_transactions: 1 }));
  }) as typeof fetch, now);
  assert.equal(data.state, "ready"); assert.equal(data.connections[0].transactionsLastSuccessfulUpdate, null); assert.equal(data.connections[0].transactionsLastFailedUpdate, null);
