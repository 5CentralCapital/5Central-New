...
/** Property statements need opening/closing property cash, receipts,
 * disbursements, and reserves. A general ledger line reader cannot prove
 * those semantics, so the report stays explicitly blocked until its
 * property-cash source is registered. */
export const PROPERTY_STATEMENT_REPORT_IDS = ["property-statement"] as const;
const COMBINED_FINANCIAL_EXECUTABLE_REPORT_IDS = COMBINED_FINANCIAL_REPORT_IDS.filter(id => id !== "property-statement") as readonly Exclude<CombinedFinancialReportId, "property-statement">[];
...
export function createCombinedFinancialReportingEngine(read: CombinedFinancialReadPort): ReportingEngine {
  return {
    ...
    reportIds: [...COMBINED_FINANCIAL_EXECUTABLE_REPORT_IDS],
    ready: true,
    async run(context): Promise<ReportingEngineResult> {
      ...
      if (source.coverage.state === "unavailable") throw new ReportingError("report_unavailable", "The accounting mirror is unavailable for the requested entity scope.", 409, { dependency: "verified_quickbooks_books" });
      const lines = scopeLines(context, reportId, source.lines);
      const missingData: ReportMissingData[] = [];
      let rows: unknown[] = [];
      if (reportId === "property-statement") {
        throw new ReportingError("report_unavailable", "Property statement requires a verified property cash statement with opening and closing cash, receipts, disbursements, and reserves.", 409, { dependency: "property_cash_statement_source" });
      }
      ...
      if (reportId.endsWith("-consolidated")) {
        ...
        rows = aggregate(lines.filter(line => line.unitId), line => `${line.unitId}:${accountKey(line, false)}`, line => ({ unitId: line.unitId, accountId: line.canonicalAccountId ?? line.accountId, sourceRealmId: line.sourceRealmId ?? null, accountName: line.accountName ?? null, propertyId: line.propertyId ?? null }));
        ...
        rows = aggregate(lines.filter(line => reportId === "portfolio-financials" || line.propertyId), line => `${reportId === "portfolio-financials" ? "portfolio" : line.propertyId}:${accountKey(line, false)}`, line => ({ propertyId: reportId === "portfolio-financials" ? null : line.propertyId, accountId: line.canonicalAccountId ?? line.accountId, sourceRealmId: line.sourceRealmId ?? null, accountName: line.accountName ?? null }));
      ...
      const columns = reportId === "budget-vs-actual"
        ...
        : reportId === "balance-sheet-by-fund-type"
          ...
          : reportId === "income-statement-by-unit"
            ? reportColumns([{ id: "propertyId", label: "Property", type: "text" }, { id: "unitId", label: "Unit", type: "text" }, { id: "accountName", label: "Account", type: "text" }, { id: "amountCents", label: "Amount", type: "money" }, { id: "currency", label: "Currency", type: "text" }])
            : reportId === "property-t12"
              ? reportColumns([{ id: "propertyId", label: "Property", type: "text" }, { id: "month", label: "Month", type: "month" }, { id: "accountName", label: "Account", type: "text" }, { id: "amountCents", label: "Amount", type: "money" }, { id: "currency", label: "Currency", type: "text" }])
              : reportId === "portfolio-financials"
                ? reportColumns([{ id: "propertyId", label: "Property", type: "text" }, { id: "accountName", label: "Account", type: "text" }, { id: "amountCents", label: "Amount", type: "money" }, { id: "currency", label: "Currency", type: "text" }])
      ...
      const result = resultFromRecords(context, rows, { source: "quickbooks_mirror_and_company_mappings", basis: context.request.basis, missingData, columns });
