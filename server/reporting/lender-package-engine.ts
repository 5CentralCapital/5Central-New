...
export function createLenderManagementPackageEngine(read: LenderPackageReadPort): ReportingEngine {
  return {
    ...
    async run(context): Promise<ReportingEngineResult> {
      ...
      if (source.coverage.state === "unavailable") throw new ReportingError("report_unavailable", "Lender package source reports and required templates are unavailable.", 409, { dependency: "required_templates" });
      const rows = source.sections.map(section => ({ section: section.section, title: section.title, reportId: section.reportId, sourceRunId: section.runId, rowCount: section.rowCount, state: section.state, reason: section.reason ?? null }));
      ...
      const result = resultFromRecords(context, rows, { source: "frozen_report_package_sections", basis: "mixed", missingData: missing, columns: reportColumns([{ id: "section", label: "Section", type: "text" }, { id: "title", label: "Report", type: "text" }, { id: "rowCount", label: "Rows", type: "integer" }, { id: "state", label: "State", type: "status" }, { id: "reason", label: "Reason", type: "text" }]) });
