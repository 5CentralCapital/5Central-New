...
export function performanceFixture(copies = 30, historyCopies = 40, singleMonthStress = false): RentOpsSnapshot {
  ...
  for (let copy = 0; copy < copies; copy++) {
    ...
    const historyDate = (value: string | null | undefined, months: number) => {
      ...
      const date=new Date(Date.UTC(year,month-1-(singleMonthStress?0:months),1));
