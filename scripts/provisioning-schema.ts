...
import {rentOpsMigrationDefinitions,type RentOpsMigrationDefinition} from '../server/rent-ops/persistence';
...
export function validateProvisioningSchemaAgainstRelease(rows:ReadonlyArray<{version:number;checksum_sha256:string}>,definitions:readonly RentOpsMigrationDefinition[]):void {
 const expected=definitions.map(row=>({version:row.version,checksum:row.checksum}));
...
/** Operator entry stays bound to the actual full checkout, never a truncated chain. */
export function validateProvisioningSchema(rows:ReadonlyArray<{version:number;checksum_sha256:string}>):void {
 validateProvisioningSchemaAgainstRelease(rows,rentOpsMigrationDefinitions());
}
