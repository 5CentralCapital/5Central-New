...
- MRA upload, mapping, preview and apply are Codex-only: the shared `/mcp` endpoint offers those tools, and attests the `mra_ingestion` capability, only when the verified OAuth token's client (`client_id`/`azp`) is listed in `RENT_OPS_MCP_MRA_CLIENT_IDS`. ChatGPT, Claude Code and any other client get the read tools only. The app displays results read-only. Ordinary records and project costs retain full manual editing.
...
- Michael approved the full design-audit fixes on September 24, including the dashboard restructure: KPIs, Needs attention, Balances due and Units by property. This supersedes the September 21 instruction to preserve the RM-style dashboard layout. Keep the global recent/frequently-used sidebar and open-record strip removed. See `docs/company/handoff-2026-09-24-design-audit.md` for the approved scope and `docs/company/existing-interface.md` for earlier context.
