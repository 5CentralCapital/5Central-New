...
import type { AccountingApi, AccountingConnection, AccountingMirrorKind, AccountingPendingBinding, AccountingScope, AccountingWorkspaceEntity, AccountingWorkspaceProps } from "./types";
...
function ConnectCard({ api, organizationId, legalEntityId }: { readonly api: AccountingApi; readonly organizationId: string; readonly legalEntityId: string }) {
  ...
  return <div className="accounting-card">
    <div className="accounting-card-header"><h2>Connect QuickBooks</h2></div>
    <div className="accounting-card-body accounting-form">
      <button className="accounting-button accounting-button-primary" disabled={starting} onClick={() => void connect()}>{starting ? "Opening QuickBooks…" : "Connect QuickBooks"}</button>
...
function ConnectionList({ connections, selected, onSelect }: { readonly connections: readonly AccountingConnection[]; readonly selected: AccountingConnection | null; readonly onSelect: (connection: AccountingConnection) => void }) {
  ...
  return <div className="accounting-card"><div className="accounting-card-header"><h2>Connections</h2></div>{connections.map(connection => <button type="button" className="accounting-connection" key={`${connection.scope.environment}:${connection.scope.realmId}`} aria-pressed={selected?.scope.realmId === connection.scope.realmId} onClick={() => onSelect(connection)}><strong>{connection.name}</strong><span>{connection.scope.environment === "production" ? "Production" : "Sandbox"}</span><span className={`accounting-status ${connection.status === "ready" ? "" : "is-muted"}`}>{connection.status === "ready" ? "Ready" : "Connected"}</span></button>)}</div>;
}
...
export function AccountingWorkspace({ organizationId, organizationName, entities, api = accountingApi }: AccountingWorkspaceProps) {
  ...
  return <div className="accounting-workspace">
    <header className="accounting-toolbar"><div><h1>Accounting</h1><p>{organizationName ?? "Company"}</p></div><div className="accounting-actions">{scope && <button className="accounting-button accounting-button-primary" disabled={syncing} onClick={() => void sync()}><RefreshCw size={14} />{syncing ? "Refreshing…" : "Refresh records"}</button>}</div></header>
    <div className="accounting-selectors"><label>Legal entity<select value={legalEntityId} onChange={event => { setLegalEntityId(event.currentTarget.value); setTab("accounts"); }}>{entityOptions.map(entity => <option key={entity.id} value={entity.id}>{entity.name}</option>)}</select></label><label>QuickBooks environment<span className="accounting-meta">{environment === "production" ? "Production" : environment === "sandbox" ? "Sandbox" : "Not configured"}</span></label><label>Currency<span className="accounting-meta">{selectedEntity?.currency ?? "—"}</span></label></div>
    ...
    <div className="accounting-grid"><aside className="accounting-sidebar">{pending.data && selectedEntity && <PendingBindingCard api={api} organizationId={organizationId} legalEntityId={legalEntityId} entityName={selectedEntity.name} pending={pending.data} onConfirmed={clearPending} />}{pending.error && <ErrorBox error={pending.error} retry={() => void pending.refetch()} />}{!connections.isLoading && <ConnectionList connections={connections.data ?? []} selected={selectedConnection} onSelect={connection => setSelectedRealmId(connection.scope.realmId)} />}{!selectedConnection && !pending.data && configuration.data?.configured && legalEntityId && <ConnectCard api={api} organizationId={organizationId} legalEntityId={legalEntityId} />}{configuration.data && !configuration.data.configured && <div className="accounting-card"><div className="accounting-card-header"><h2>QuickBooks is not configured</h2></div><div className="accounting-card-body">QuickBooks connection isn't available.</div></div>}</aside><main className="accounting-main">{scope ? <><nav className="accounting-tabs" aria-label="Accounting records">{[...mirrorTabs, ["transactions", "Transactions"] as const].map(([value, label]) => <button type="button" className={`accounting-tab ${tab === value ? "is-selected" : ""}`} key={value} onClick={() => setTab(value)}>{label}</button>)}</nav>{tab === "transactions" ? <TransactionTable api={api} organizationId={organizationId} scope={scope} /> : mirrors.isLoading ? <div className="accounting-empty">Loading {tab}…</div> : mirrors.error ? <ErrorBox error={mirrors.error} retry={() => void mirrors.refetch()} /> : <MirrorTable items={mirrors.data ?? []} kind={tab} />}</> : <div className="accounting-empty"><strong>Choose a QuickBooks connection.</strong></div>}</main></div>
  </div>;
}
