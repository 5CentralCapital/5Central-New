...
import { chartLineSegments, dashboardChartSeries, type TrendMetric, type TrendMeasure } from "./dashboard-model";
...
export function DashboardChart({ metric, data, loading, error, onRetry }: { metric: TrendMetric; data?: DashboardTrends; loading: boolean; error?: string; onRetry: () => void }) {
  ...
  const [measure, setMeasure] = useState<TrendMeasure>("rate");
  ...
  if (error) body = <div className="rmd-message" role="alert">History could not be loaded. <button onClick={onRetry}>Retry</button></div>;
    ...
    {valuesOpen ? <div className="rmd-table-scroll rmd-chart-values"><table><thead><tr><th>Month</th>{visible.map(item => <th key={item.id} className="number">{item.name}</th>)}</tr></thead><tbody>
      {data.months.map((month, index) => <tr key={month.month}><td>{month.asOfDate}</td>{visible.map(item => <td key={item.id} className="number">{format(item.values[index])}</td>)}</tr>)}
      ...
      <svg className="rmd-chart" viewBox="0 0 640 223" role="img" aria-label={`${labels[metric]}, 12 months ending ${data.asOfDate}`} onMouseLeave={() => setHover(null)}>
        <title>{labels[metric]} — 12 months ending {data.asOfDate}</title>
        ...
        {data.months.map((month, index) => <rect key={month.month} x={x(index) - 22} y="25" width="44" height="150" fill="transparent" tabIndex={0} role="button" aria-label={`${month.asOfDate}: ${visible.map(item => `${item.name} ${format(item.values[index])}`).join(", ")}`} onFocus={() => setHover(index)} onBlur={() => setHover(null)} onMouseEnter={() => setHover(index)} onClick={() => setHover(index)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setHover(index); } }} />)}
      </svg>
    </div>}
    <div className="rmd-chart-readout" aria-live="polite">
      {hover !== null ? <><strong>{data.months[hover].asOfDate}</strong>{visible.map(item => <span key={item.id}>{item.name}: <b>{item.values[hover] === null ? "History incomplete" : format(item.values[hover])}</b></span>)}</>
        : <><span>12 months · through {data.asOfDate}</span>{missing > 0 && <span className="rmd-incomplete">{missing} {missing === 1 ? "month has" : "months have"} incomplete history</span>}{loading && <span>Refreshing…</span>}</>}
    </div>
  </>;
  return <section className="rmd-panel rmd-trend-panel" aria-label={labels[metric]}>
    ...
    <div className="rmd-chart-filters">
      ...
      {metric !== "rent" && <select aria-label={`${labels[metric]} measure`} value={measure} onChange={event => setMeasure(event.target.value as TrendMeasure)}><option value="rate">Percent</option><option value="units">Units</option></select>}
