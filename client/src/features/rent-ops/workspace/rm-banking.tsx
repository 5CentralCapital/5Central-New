...
export function RmBanking() {
  ...
  return <section className="rmb" aria-label="Banking"><div className="rmb-heading"><h2>Bank accounts</h2><button type="button" disabled={bank.isFetching} onClick={() => void bank.refetch()}>{bank.isFetching ? "Checking…" : "Refresh"}</button></div>
    {bank.isLoading ? <p role="status">Loading bank accounts…</p> : bank.isError ? <p role="alert">Banking could not be loaded. Try refreshing.</p> : data?.state === "unconfigured" ? <p>Banking is not connected to this workspace yet.</p> : <>
      {data && <p className="rmb-status">Fetched {new Date(data.fetchedAt).toLocaleString()}. Bank reads refresh at most every five minutes.</p>}
      ...
      <div className="rmb-accounts">{data?.connections.map(connection => <section className="rmb-connection" key={connection.id}><h2>{connection.name}</h2>{connection.balancesState !== "ready" && <p>Live balances unavailable.</p>}{connection.transactionsState !== "ready" && <p>{connection.transactionsState === "partial" ? "Transaction list is incomplete." : "Transactions unavailable."}</p>}{connection.accounts.map(account => <article key={account.id}><h3>{account.name} {account.mask ? `••${account.mask}` : ""}</h3><span>{account.type} · {account.currency ?? "Currency unavailable"}</span><dl><div><dt>Current balance</dt><dd>{money(account.currentCents, account.currency)}</dd></div><div><dt>Available balance</dt><dd>{money(account.availableCents, account.currency)}</dd></div></dl></article>)}{connection.accounts.length === 0 && connection.balancesState === "ready" && <p>No accounts returned.</p>}</section>)}</div>
      <div className="rmb-heading"><h2>Recent transactions</h2><div className="rmb-filters"><label>Account <select value={selected} onChange={event => setSelected(event.target.value)}><option value="all">All accounts</option>{accounts.map(account => <option value={account.id} key={account.id}>{account.connection} · {account.name} {account.mask ? `••${account.mask}` : ""}</option>)}</select></label><label>Status <select value={status} onChange={event => setStatus(event.target.value)}><option value="all">All</option><option value="posted">Posted</option><option value="pending">Pending</option></select></label></div></div>
