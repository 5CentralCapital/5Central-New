...
import "./editor.css";
import { missingLabel } from "@shared/review-cases/display-labels";
      ...
      <section ref={dialog} onKeyDown={(event) => {
        ...
        <form onSubmit={submit}>
          ...
          <fieldset disabled={saving} className="rm-dialog-fields"><legend className="sr-only">Record details</legend><div className="rm-dialog-grid">
            {fields.filter(field => !field.locked).map((field) => <label key={field.name} className={field.type === "textarea" ? "wide" : ""}>{field.label}{field.required && <sup> *</sup>}{field.locked ? <span className="rm-charge-context-value">{field.options?.find(([id]) => id === String(values[field.name] ?? ""))?.[1] ?? missingLabel(field.label)}</span> : field.type === "select" ? <select required={field.required} value={String(values[field.name] ?? "")} onChange={(event) => change(field.name, event.target.value)}><option value="">Select…</option>{field.options?.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select> : field.type === "textarea" ? <textarea required={field.required} value={String(values[field.name] ?? "")} onChange={(event) => change(field.name, event.target.value)} /> : field.type === "checkbox" ? <select required={field.required} value={typeof values[field.name] === "boolean" ? String(values[field.name]) : ""} onChange={(event) => { const selected = event.target.value; setChangedFields(current => new Set([...Array.from(current), field.name])); setValues(current => ({ ...current, [field.name]: selected === "" ? undefined : selected === "true" })); }}><option value="" disabled={typeof initialValues[field.name] === "boolean"}>Unknown / not specified</option><option value="true">Yes</option><option value="false">No</option></select> : <input required={field.required} type={field.type ?? "text"} step={field.type === "number" ? "0.01" : undefined} value={String(values[field.name] ?? "")} onChange={(event) => change(field.name, event.target.value)} />}{fieldErrors[field.name] && <span className="rm-dialog-field-error" role="alert">{fieldErrors[field.name]}</span>}</label>)}
          </div>
          {action === "post-ledger-transaction" && values.kind === "charge" && <ProrateRent amount={String(values.amountDollars??"")} date={String(values.postedOn??"")} onApply={value=>{change("amountDollars",value);change("category","base_rent");}}/>}
          </fieldset>
