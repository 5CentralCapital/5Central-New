...
export function RmDashboard({ snapshot, filters, onReport, onOpenTenant, onOpenUnit, onOpenProperty, previews, refreshing = false, onManageMoves }: DashboardWorkspaceProps) {
  ...
  return <section className="rm-dashboard-workspace rmd-dashboard" aria-label="Rent Operations dashboard">
    ...
    <div className="rmd-top-grid">
      ...
      <Panel title="Delinquency List" className="rmd-delinquency" onOpen={() => onReport("delinquency")}><Table rows={dueRows} empty="No balances due." columns={[{ key: "tenantName", label: "Name", render: personLink }, propertyColumn, unitColumn, { key: "oldestUnpaidRentOn", label: "Date" }, { ...amountColumn("operationalBalanceCents", "Amount"), render: row => numeric(row.operationalBalanceCents) ? <EntityLink personId={String(row.personId ?? "")} tab="ledger" onOpen={onOpenTenant}>{money(row.operationalBalanceCents)}</EntityLink> : "Needs review" }]} footer={<><span>{dueRows?.length ?? "—"} accounts</span><strong>{dueRows?.some(row => !numeric(row.operationalBalanceCents)) ? "Total needs review" : money(total(dueRows, "operationalBalanceCents"))}</strong></>} /></Panel>
