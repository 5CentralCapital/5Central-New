...
import { handleRentOpsMutationError, refreshRentOpsAfterConflict, sectionCreateAction, scheduledRentNeedsReview } from "./ui";
...
function TenantDetail({ tenant, tab, onTab, onEdit, onChanged, chargeDefinitions }: { chargeDefinitions: AdminSnapshot["chargeDefinitions"]; onChanged: () => void; tenant: TenantView; tab: TenantTab; onTab: (tab: TenantTab) => void; onEdit: (action: QuickAction, values: FormValues) => void }) {
  const items: Record<TenantTab, unknown[]> = {
    ...
    ledger: (tenant.ledger ?? []).map(row => ({ ...row.transaction, allocatedCents: row.allocatedCents, openCents: row.openCents, runningBalanceCents: row.runningBalanceCents })),
...
export default function RentOpsWorkspace() {
  ...
  return <main className="rent-ops-shell">
    <aside className="ro-sidebar"><div className="ro-brand"><span>5C</span><div><strong>Rent Operations</strong><small>Single-admin workspace</small></div></div><a className="ro-back-link" href="/">← 5Central website</a><nav>{SECTIONS.map(({ key, label, icon: Icon }) => <button key={key} className={section === key ? "active" : ""} onClick={() => setSection(key)}><Icon />{label}</button>)}</nav><div className="ro-source"><span className={source === "live" ? "live" : "demo"} />{source === "live" ? "Live operational data" : "Synthetic development data"}<small>Updated {new Date(snapshot.generatedAt).toLocaleString()}</small><button className="ro-logout" type="button" onClick={() => { void rentOpsAuthClient.logout(); }}><LogOut /> Sign out</button></div></aside>
