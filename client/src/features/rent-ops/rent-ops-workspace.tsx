      ...
      <table className="ro-table">
        ...
        <tbody>
          {report.rows.map((row, index) => (
            <tr key={reportRowKey(report, row, index)} tabIndex={onRow ? 0 : undefined} className={onRow ? "clickable" : ""} onClick={() => onRow?.(row)} onKeyDown={(event) => { if (onRow && (event.key === "Enter" || event.key === " ")) onRow(row); }}>
...
const ACTION_LABELS: Partial<Record<QuickAction, string>> = {
  ...
  "post-ledger-transaction": "Post charge, payment, or credit",
...
function ActionDialog({ action, snapshot, initialValues = {}, onClose, onSaved, onConflict }: { action: QuickAction; snapshot: AdminSnapshot; initialValues?: FormValues; onClose: () => void; onSaved: (message: string) => void; onConflict?: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await postRentOpsMutation({ action, payload: mutationPayload(action, values, initialValues) });
      onSaved(`${ACTION_LABELS[action]} completed.`); onClose();
    }
...
function Reports({ snapshot, filters, selected, onSelect }: { snapshot: AdminSnapshot; filters: ViewFilters; selected: ReportKey; onSelect: (report: ReportKey) => void }) {
  const report = filterReportRows(snapshot.reports[selected], filters, snapshot);
  return <><div className="ro-report-picker" aria-label="Report list">{REPORT_KEYS.map((key) => <button key={key} className={selected === key ? "active" : ""} onClick={() => onSelect(key)}><strong>{REPORT_LABELS[key]}</strong><span>{snapshot.reports[key].description}</span></button>)}</div><section className="ro-panel"><div className="ro-panel-heading"><div><span className="eyebrow">Fixed operational report</span><h2>{report.label}</h2><p>{report.description}</p></div><div className="ro-actions"><button className="secondary" onClick={() => saveTextFile(`rent-ops-${selected}-${filters.asOfDate}.csv`, reportToCsv(report))}><Download /> CSV</button><button className="secondary" onClick={() => window.print()}><Printer /> Print</button></div></div><DataTable report={report} /><p className="source-note">{report.sourceNote}</p></section></>;
}

function TenantDetail({ tenant, tab, onTab, onEdit }: { tenant: TenantView; tab: TenantTab; onTab: (tab: TenantTab) => void; onEdit: (action: QuickAction, values: FormValues) => void }) {
  const items: Record<TenantTab, unknown[]> = {
    summary: [tenant.person, tenant.property, tenant.unit],
...
function PropertyPanels({ snapshot, filters, onEdit }: { snapshot: AdminSnapshot; filters: ViewFilters; onEdit: (action: QuickAction, initialValues: FormValues) => void }) {
  const properties = snapshot.snapshot.properties.filter((property) => filters.propertyId === "all" || property.id === filters.propertyId);
  return <div className="ro-property-grid">{properties.map((property) => {
    ...
    return <section className="ro-panel" key={property.id}>
      <div className="ro-panel-heading">
        ...
        <div className="ro-actions"><strong>{units.length} units</strong><button className="secondary" onClick={() => onEdit("save-property", { id: property.id, revision: property.recordRevision ?? 1, name: property.name, slug: property.slug, address1: property.address?.line1 ?? "", city: property.address?.city ?? "", stateCode: property.address?.state ?? "", postalCode: property.address?.postalCode ?? "", propertyType: property.propertyType, propertyState: property.state, operatingContact: property.operatingContact ?? "" })}>Edit property</button></div>
      </div>
      <div className="ro-unit-list">{units.map((unit) => <div key={unit.id}>
        <strong>{unit.unitNumber ?? "Needs review"}</strong><span>{unit.bedrooms ?? "Needs review"} bd · {unit.bathrooms ?? "Needs review"} ba</span><span className={`status ${unit.readiness ?? "unknown"}`}>{formatCell(unit.readiness, "readiness")}</span><span>{money(unit.marketRentCents)}</span>
        <button className="secondary" onClick={() => onEdit("save-unit", { id: unit.id, revision: unit.recordRevision ?? 1, propertyId: unit.propertyId, unitNumber: unit.unitNumber, bedrooms: unit.bedrooms == null ? "" : String(unit.bedrooms), bathrooms: unit.bathrooms == null ? "" : String(unit.bathrooms), marketRentDollars: unit.marketRentCents == null ? "" : String(unit.marketRentCents / 100), readiness: unit.readiness, listing: unit.listing })}>Edit</button>
      </div>)}</div>
    </section>;
  })}</div>;
}

function ApplicantCard({ application, onChanged, onError, onConflict, onConvert, onOpen }: { application: AdminSnapshot["applicants"][number]; onChanged: (message: string) => void; onError: (message: string) => void; onConflict: () => void; onConvert: () => void; onOpen: (applicationId: string) => void }) {
...
export default function RentOpsWorkspace() {
  ...
  function openDrilldown(metric: keyof AdminSnapshot["summary"]["drilldowns"], fallback: ReportKey) {
    ...
    setFilters((current) => ({ ...current, status: occupancy ?? current.status }));
    setSection("reports");
  }
  ...
  return <main className="rent-ops-shell">
    <aside className="ro-sidebar"><div className="ro-brand"><span>5C</span><div><strong>Rent Operations</strong><small>Single-admin workspace</small></div></div><a className="ro-back-link" href="/admin">← Back to Admin</a><nav>{SECTIONS.map(({ key, label, icon: Icon }) => <button key={key} className={section === key ? "active" : ""} onClick={() => setSection(key)}><Icon />{label}</button>)}</nav><div className="ro-source"><span className={source === "live" ? "live" : "demo"} />{source === "live" ? "Live operational data" : "Synthetic development data"}<small>Updated {new Date(snapshot.generatedAt).toLocaleString()}</small><button className="ro-logout" type="button" onClick={() => { void rentOpsAuthClient.logout(); }}><LogOut /> Sign out</button></div></aside>
    <div className="ro-main"><header className="ro-topbar"><div><span className="eyebrow">Portfolio records</span><h1>{SECTIONS.find((item) => item.key === section)?.label}</h1></div><button className="primary" onClick={() => openAction(section === "properties" ? "save-property" : section === "income" ? "post-ledger-transaction" : section === "documents" ? "save-activity" : "save-person")}><Plus /> Add record</button></header>
      ...
      {section === "applicants" && <><div className="ro-inline-actions"><span>All decisions are manual. This tool does not score or screen applicants.</span></div><div className="ro-card-grid">{snapshot.applicants.map((application) => <ApplicantCard key={application.id} application={application} onChanged={(message) => { setNotice(message); void load(); }} onError={setError} onConflict={() => refreshRentOpsAfterConflict({ closeEditor: () => undefined, clearEditor: () => undefined, showNotice: setNotice, reload: load })} onConvert={() => openAction("convert-application", { applicationId: application.id })} onOpen={setSelectedApplicationId} />)}</div></>}
