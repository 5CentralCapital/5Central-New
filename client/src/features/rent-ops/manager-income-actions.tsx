import { STATUS_NOT_SET_LABEL } from '@shared/review-cases/display-labels';
import { useState, type FormEvent } from "react";
import { rentOpsAuthClient } from "./auth";
import { requireCentsInput } from "./money";
import type { AdminLedgerTransactionView, AdminSnapshot } from "./types";
import { formatMoney } from "./workspace/display";
...
/** Allocation row label; a charge with no known amount reads "Unknown", never $0.00. */
export function allocationChargeLabel(charge: Pick<AdminLedgerTransactionView, "postedOn" | "description" | "amountCents">): string {
  return `${charge.postedOn} · ${charge.description} · ${formatMoney(charge.amountCents)}`;
}

export function ManagerIncomeActions({ snapshot, businessDate, onSaved, propertyId, onRequestPaymentContext, paymentContextLoading=false }: { snapshot: AdminSnapshot; businessDate?: string; onSaved: () => Promise<void>; propertyId: string; onRequestPaymentContext?: () => void; paymentContextLoading?: boolean }) {
  ...
  return <section className="ro-panel"><div className="ro-panel-heading"><h2>Income actions</h2><div className="ro-actions"><button className="primary" onClick={() => { setMode("payment"); setMessage(""); onRequestPaymentContext?.(); }}>Record manual payment</button><button className="secondary" onClick={() => { setMode("definitions"); setMessage(""); }}>Charge types</button></div></div>
    {message && <p className="ro-panel-message" role="status">{message}</p>}
    {mode && <form key={`${mode}:${editingDefinition?.id ?? "new"}`} className="ro-income-form" onSubmit={submit}><fieldset disabled={busy || (mode === "payment" && paymentContextLoading)}><div className="ro-form-grid">
      {mode === "payment" ? <><label>Tenancy<select required value={tenancyId} onChange={event => { setTenancyId(event.target.value); setAllocations({}); }}><option value="">Select tenancy…</option>{tenancies.map(row => { const person = snapshot.snapshot.people.find(person => person.id === row.primaryPersonId); const unit = snapshot.snapshot.units.find(unit => unit.id === row.unitId); const property = snapshot.snapshot.properties.find(property => property.id === row.propertyId); return <option key={row.id} value={row.id}>{property?.name} / {unit?.unitNumber} · {person?.firstName} {person?.lastName} · {row.status}</option>; })}</select></label><label>Amount<input name="amount" required type="number" step="0.01" min="0.01" /></label><label>Received date<input name="postedOn" required type="date" defaultValue={businessDate} /></label><label>Method<select name="paymentMethod">{["ach", "cash", "check", "money_order", "zelle", "other"].map(value => <option key={value}>{value}</option>)}</select></label><label>Category<select name="category">{["base_rent", "recurring_fee", "one_time_fee", "unapplied_cash", "other"].map(value => <option key={value} value={value}>{label(value)}</option>)}</select></label><label>Description / reference<input name="description" required maxLength={240} /></label></> : <><label>Charge type name<input name="displayName" required maxLength={240} defaultValue={editingDefinition?.displayName ?? ""} /></label><label>Category<select name="category" disabled={Boolean(editingDefinition)} defaultValue={editingDefinition?.category ?? "base_rent"}>{["base_rent", "recurring_fee", "one_time_fee", "subsidy", "security_deposit", "refundable_pet_deposit", "move_in_funds", "unapplied_cash", "other"].map(value => <option key={value} value={value}>{label(value)}</option>)}</select></label>{editingDefinition && <label>Status<select name="active" defaultValue={String(editingDefinition.active ?? "")} required><option value="">{STATUS_NOT_SET_LABEL}</option><option value="true">Active</option><option value="false">Inactive</option></select></label>}</>}
    </div>{mode === "payment" && <>{paymentContextLoading && <p className="ro-panel-message" role="status">Loading current charges…</p>}<p>Record money already received. Allocate to charges below; any remainder stays unapplied.</p>{charges.map(charge => <label className="ro-allocation" key={charge.id}>{allocationChargeLabel(charge)}<input aria-label={`Allocate to ${charge.description}`} type="number" min="0.01" step="0.01" value={allocations[charge.id!] ?? ""} onChange={event => setAllocations(current => ({ ...current, [charge.id!]: event.target.value }))} /></label>)}</>}<div className="ro-actions"><button className="primary" disabled={busy}>{busy ? "Saving…" : mode === "payment" ? "Record payment" : editingDefinition ? "Save charge type" : "Add charge type"}</button><button type="button" className="secondary" onClick={() => setMode(undefined)}>Close</button></div></fieldset></form>}
    {mode === "definitions" && <div className="ro-table-wrap"><table className="ro-table"><thead><tr><th>Charge type</th><th>Category</th><th>Status</th><th>Action</th></tr></thead><tbody>{snapshot.chargeDefinitions.map(row => <tr key={row.id}><td>{row.displayName}</td><td>{label(row.category ?? "unknown")}</td><td>{row.active == null ? STATUS_NOT_SET_LABEL : row.active ? "Active" : "Inactive"}</td><td><button className="secondary" disabled={!row.id || !row.recordRevision || busy} onClick={() => setEditingDefinition(row)}>Edit</button></td></tr>)}</tbody></table></div>}
  </section>;
}
