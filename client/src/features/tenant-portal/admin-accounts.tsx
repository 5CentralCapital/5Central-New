...
import { Copy, Link, Loader2, Mail, RefreshCw, ShieldOff } from "lucide-react";
...
export function TenantPortalAccountsPanel({ personId, personName, email }: { personId: string; personName: string; email?: string }) {
  ...
  async function applyAction() {
    ...
    try {
      ...
      if (action === "reissue") setLink({ url: activationUrl(result.activationPath, window.location.origin), expiresAt: result.expiresAt, email: account.email });
        ...
        if (result.delivery !== "accepted") throw new Error("Email acceptance could not be confirmed. Refresh before trying again.");
        setNotice(`The email provider accepted the ${account.status === "active" ? "password reset" : "invitation"} email for ${account.email}. Inbox delivery is not yet confirmed.`);
  ...
  return <section className="tp-admin-accounts" aria-labelledby={`tp-accounts-${personId}`}><div className="tp-admin-heading"><h3 id={`tp-accounts-${personId}`}>Tenant portal account</h3><button type="button" className="tp-secondary" disabled={busy} onClick={refresh} aria-label="Refresh tenant accounts"><RefreshCw className={busy ? "tp-spin" : ""} /></button></div>
    ...
    {data && !data.deliveryAvailable && <p className="tp-notice">Email delivery is not configured. You can prepare an account link, but invitation and password reset emails are unavailable.</p>}
    {eligible.some(item => item.paymentReviewReason === "assistance_responsibility_unverified") && <p className="tp-notice">Online payment is on hold while tenant and housing assistance responsibility is confirmed. Account access and lease documents remain available.</p>}
    {accounts.map((account) => { const selected = eligible.find((item) => item.tenancyId === account.tenancyId); return <div key={account.id} className="tp-admin-account"><div><strong>{account.email}</strong><span className="tp-admin-status">{account.status}</span><small>{selected ? `${selected.propertyName} · Unit ${selected.unitNumber}` : `Tenancy ${account.tenancyId}`}</small></div><div className="tp-actions">{account.status !== "revoked" && <button type="button" className="tp-primary" disabled={busy || !data?.deliveryAvailable} onClick={() => setPendingAction({ account, action: "send-link" })}><Mail />{account.status === "active" ? "Send password reset" : "Send invitation"}</button>}<button type="button" className="tp-secondary" disabled={busy} onClick={() => setPendingAction({ account, action: "reissue" })}><Link />New access link</button>{account.status !== "revoked" && <button type="button" className="tp-secondary tp-danger" disabled={busy} onClick={() => setPendingAction({ account, action: "revoke" })}><ShieldOff />Revoke</button>}</div></div>; })}
    {pendingAction && <div className="tp-confirm"><p>{pendingAction.action === "send-link" ? `Send a ${pendingAction.account.status === "active" ? "password reset" : "portal invitation"} email to ${pendingAction.account.email}?` : pendingAction.action === "revoke" ? `Revoke ${pendingAction.account.email}'s portal access and end their active sessions?` : `Create a new access link for ${pendingAction.account.email}? Previous links and sessions will stop working.`}</p><div className="tp-actions"><button type="button" className="tp-primary" disabled={busy} onClick={applyAction}>{busy && <Loader2 className="tp-spin" />}{pendingAction.action === "send-link" ? "Send email" : pendingAction.action === "revoke" ? "Revoke access" : "Create new link"}</button><button type="button" className="tp-secondary" disabled={busy} onClick={() => setPendingAction(undefined)}>Cancel</button></div></div>}
