...
function Axis({ scale, format = compactDollars }: { scale: Scale; format?: (value: number) => string }) {
...
export function LineChart({ title, periods, series, floorCents, periodLabel = dateLabel, onSelect, selectedKey, footer }: {
  ...
  const x = (index: number) => (periods.length <= 1 ? (LEFT + WIDTH - RIGHT) / 2 : LEFT + (index * (WIDTH - RIGHT - LEFT)) / (periods.length - 1));
  const labelEvery = Math.max(1, Math.ceil(periods.length / 7));
  return <figure className="fc-chart" aria-labelledby={titleId}>
    <figcaption id={titleId} className="fc-chart-title">{title}</figcaption>
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${title}. Values are listed in the table below.`} preserveAspectRatio="xMidYMid meet">
      <Axis scale={scale} />
      {floor !== null && <g className="fc-floor">
        <line x1={LEFT} x2={WIDTH - RIGHT} y1={scale.y(floor)} y2={scale.y(floor)} />
        <text x={WIDTH - RIGHT + 6} y={scale.y(floor)} dy="0.32em">Reserve floor</text>
      </g>}
      ...
      {series.map((item, seriesIndex) => {
        ...
        return <g key={item.id} className={`fc-series fc-series--${item.tone}${item.dashed ? " fc-series--dashed" : ""}`}>
          <path d={path} />
          {lastIndex >= 0 && <text className="fc-direct-label" x={x(lastIndex) + 8} y={scale.y(numeric[seriesIndex]![lastIndex]!)} dy="0.32em">{item.label}</text>}
          {numeric[seriesIndex]!.map((value, index) => value === null ? null : <circle key={periods[index]!.key} cx={x(index)} cy={scale.y(value)} r={periods[index]!.key === selectedKey ? 4.5 : 3}
            className={floor !== null && item.id === "available" && value < floor ? "fc-point fc-point--low" : "fc-point"} />)}
        </g>;
      })}
      {onSelect && periods.map((period, index) => {
        const width = periods.length <= 1 ? WIDTH - LEFT - RIGHT : (WIDTH - RIGHT - LEFT) / (periods.length - 1);
...
export function WaterfallChart({ title, openingCents, closingCents, steps, onSelect }: {
  ...
  const slot = (WIDTH - LEFT - 24) / bars.length;
  return <figure className="fc-chart" aria-labelledby={titleId}>
    <figcaption id={titleId} className="fc-chart-title">{title}</figcaption>
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT + 24}`} role="img" aria-label={`${title}. Values are listed beside the chart.`}>
      <Axis scale={scale} />
      {bars.map((bar, index) => {
...
export function BarLineChart({ title, months, bars, barLabel, line, lineLabel, onSelect }: {
  ...
  return <figure className="fc-chart" aria-labelledby={titleId}>
    <figcaption id={titleId} className="fc-chart-title">{title}</figcaption>
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${title}. Values are listed in the table below.`}>
      <Axis scale={scale} />
      {line && <g className="fc-axis" aria-hidden="true">
        {[0, 5_000, 10_000].map(bps => <text key={bps} x={WIDTH - RIGHT + 8} y={percentY(bps)} dy="0.32em">{bps / 100}%</text>)}
      </g>}
      ...
      {line && <g className="fc-series fc-series--gold"><path d={linePath} />
        {lineLabel && <text className="fc-direct-label" x={WIDTH - RIGHT + 8} y={TOP + 2}>{lineLabel}</text>}</g>}
  ...
  return <figure className="fc-chart" aria-labelledby={titleId}>
    ...
    <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${title}. Values are listed in the table below.`}>
      ...
      <g className="fc-legend-inline" aria-hidden="true">
        <rect className="fc-stack fc-stack--ink" x={WIDTH - RIGHT + 8} y={TOP} width={10} height={10} /><text x={WIDTH - RIGHT + 22} y={TOP + 9}>Maturing</text>
        <rect className="fc-stack fc-stack--muted" x={WIDTH - RIGHT + 8} y={TOP + 18} width={10} height={10} /><text x={WIDTH - RIGHT + 22} y={TOP + 27}>Amortizing</text>
      </g>
    </svg>
  </figure>;
}
...
export function CompositionChart({ title, assets, claims, onSelect }: { title: string; assets: readonly CompositionPart[]; claims: readonly CompositionPart[]; onSelect?: (key: string) => void }) {
  ...
  return <figure className="fc-chart" aria-labelledby={titleId}>
    <figcaption id={titleId} className="fc-chart-title">{title}</figcaption>
    <svg viewBox={`0 0 ${WIDTH} 96`} role="img" aria-label={`${title}. Values are listed in the table below.`}>
      {row(assets, 8, "Assets")}
