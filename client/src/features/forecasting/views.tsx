...
export function DebtView({ result, onDrill }: { result: ForecastResultView; onDrill: Drill }) {
  ...
  const repaymentPrincipal = sumCents(payments.filter(row => row.kind !== "draw").map(row => row.principalCents));
  const fundingPrincipal = sumCents(payments.filter(row => row.kind === "draw").map(row => row.principalCents.replace(/^-/, "")));
    ...
    {year && <div className="fc-scroll" role="region" aria-label={overdue ? "Overdue loan balances" : `Loan payments in ${year}`} tabIndex={0}>
      <table className="rm-table fc-table fc-table--compact">
        ...
        <tbody>{payments.map(row => <tr key={`${row.loan}-${row.date}-${row.kind}`}><td>{dateLabel(row.date, "long")}</td><td>{row.loan}</td><td>{kindLabel[row.kind]}</td>
          <td className="fc-num">{money(row.interestCents, currency)}</td><td className="fc-num">{money(row.principalCents.replace(/^-/, ""), currency)}</td><td className="fc-num">{money(row.balanceCents, currency)}</td></tr>)}</tbody><tfoot><tr className="fc-row--total"><th scope="row">Shown: {payments.length} payment{payments.length === 1 ? "" : "s"}</th><td colSpan={2}>Period totals</td><td className="fc-num">{money(paymentInterest, currency)}</td><td className="fc-num">Repayments {money(repaymentPrincipal, currency)} · Funding {money(fundingPrincipal, currency)}</td><td className="fc-num">—</td></tr></tfoot>
      </table>
    </div>}
    <Scroll label="Loans">
      <table className="rm-table fc-table">
    ...
    {(capital.refinances.length > 0 || capital.sales.length > 0) && <section aria-label="Capital events">
      {capital.refinances.length > 0 && <Scroll label="Refinances"><table className="rm-table fc-table">
        ...
        <tbody>{capital.refinances.map(item => <tr key={item.id}><th scope="row">{item.label}{item.excluded && <span className="rm-status rm-status--unknown fc-tag">Excluded</span>}</th><td>{dateLabel(item.closeOn, "long")}</td>
          ...
          <td className="fc-num">{moneyWhole(item.reservesCents, currency)}</td><td className="fc-num fc-num--strong">{moneyWhole(item.netUsableCents, currency)}</td></tr>)}</tbody><tfoot><tr className="fc-row--total"><th scope="row">Shown: {capital.refinances.length} refinances</th><td>Included modeled totals</td><td className="fc-num">{money(sumCents(capital.refinances.filter(item => !item.excluded).map(item => item.grossProceedsCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.refinances.filter(item => !item.excluded).map(item => item.payoffCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.refinances.filter(item => !item.excluded).map(item => item.costsCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.refinances.filter(item => !item.excluded).map(item => item.reservesCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.refinances.filter(item => !item.excluded).map(item => item.netUsableCents)), currency)}</td></tr></tfoot>
      </table></Scroll>}
      {capital.sales.length > 0 && <Scroll label="Sales"><table className="rm-table fc-table">
        ...
        <tbody>{capital.sales.map(item => <tr key={item.id}><th scope="row">{item.label}{item.excluded && <span className="rm-status rm-status--unknown fc-tag">Excluded</span>}</th><td>{dateLabel(item.closeOn, "long")}</td>
          ...
          <td className="fc-num fc-num--strong">{moneyWhole(item.netProceedsCents, currency)}</td></tr>)}</tbody><tfoot><tr className="fc-row--total"><th scope="row">Shown: {capital.sales.length} sales</th><td>Included modeled totals</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.priceCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.sellingCostsCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.netBookValueCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.gainCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.payoffCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.depositsTransferredCents)), currency)}</td><td className="fc-num">{money(sumCents(capital.sales.filter(item => !item.excluded).map(item => item.netProceedsCents)), currency)}</td></tr></tfoot>
      </table></Scroll>}
    </section>}
  </div>;
}
