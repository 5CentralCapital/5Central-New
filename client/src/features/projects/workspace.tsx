...
function qboIdentityDefaults(project: ProjectDetail): QboIdentityFormValues {
  const values: { nativeProjectId: string; customerId: string; environment: "sandbox" | "production"; realmId: string } = { nativeProjectId: "", customerId: "", environment: "production", realmId: "" };
  for (const identity of project.qboProjectIdentities) {
    if (identity.recordKind === "Project") values.nativeProjectId = identity.externalId;
    if (identity.recordKind === "Customer") values.customerId = identity.externalId;
    values.environment = identity.environment;
    values.realmId = identity.realmId;
  }
  return values;
}
