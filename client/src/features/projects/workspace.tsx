...
type Tab = "overview" | "scope" | "schedule" | "costs";
type EditorState =
...
export function ProjectWorkspace({ organizationId, organizationName, entities = [], api: apiProp, initialProjectId, onNavigate }: ProjectWorkspaceProps) {
  ...
  useEffect(() => { if (initialProjectId && initialProjectId !== selectedId) setSelectedId(initialProjectId); }, [initialProjectId, selectedId]);
  ...
  const selectProject = (id: string) => { setSelectedId(id); setTab("overview"); setNotice(undefined); onNavigate?.(id); };
