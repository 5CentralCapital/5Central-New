...
/** Group signed cents by currency without converting through a JavaScript number. */
export function sumMoneyByCurrency(values: readonly MoneyValue[]): readonly CurrencyTotal[] {
  ...
  for (const value of values) {
    const rawCurrency = value.currency?.trim().toUpperCase() ?? "";
    const currency = /^[A-Z]{3}$/.test(rawCurrency) ? rawCurrency : "Unknown currency";
  ...
  return Array.from(totals.entries()).map(([currency, value]) => ({ currency, cents: value.total.toString(), knownCount: value.knownCount, unknownCount: value.unknownCount })).sort((left, right) => left.currency.localeCompare(right.currency));
}
