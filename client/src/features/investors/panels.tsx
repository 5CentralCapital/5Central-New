...
function moneyTotals(values: readonly MoneyValue[]): string {
  const totals = sumMoneyByCurrency(values);
  if (!totals.length) return "—";
  return totals.map(total => total.knownCount === 0 ? `Unknown${total.unknownCount > 1 ? ` (${total.unknownCount})` : ""}` : `${money(total.cents, total.currency)}${total.unknownCount ? ` + ${total.unknownCount} unknown` : ""}`).join(" · ");
}
...
export function PaymentCalendarPanel({ api, organizationId, detail, month }: { api: InvestorsApi; organizationId: string; detail: InvestorDetail; month: string }) {
  ...
  const calendarExpected = moneyTotals(calendarItems.map(item => ({ cents: item.expectedCents ?? item.knownMinimumCents, currency: item.currency })));
  const calendarRemaining = moneyTotals(calendarItems.map(item => ({ cents: item.remainingCents, currency: item.currency })));
  return <div className="investors-card" aria-label="Payment calendar">
    ...
    {items && <div className="investors-list-summary">Shown: {items.length} obligations · Filtered totals: expected {calendarExpected} · remaining {calendarRemaining}{items.some(item => item.expectedCents === null) ? ` · ${items.filter(item => item.expectedCents === null).length} expected amount${items.filter(item => item.expectedCents === null).length === 1 ? "" : "s"} partial` : ""}</div>}
  </div>;
}

const CAPITAL_KINDS = new Set(["contribution", "return_of_capital", "distribution"]);
const DEBT_KINDS = new Set(["private_loan", "member_loan"]);
const EQUITY_KINDS = new Set(["equity", "preferred_equity"]);
...
/** Capital contributed, returned and distributed by equity instrument, with debt principal kept separate. */
export function CapitalPanel({ detail }: { detail: InvestorDetail }) {
  ...
  const instrumentsById = new Map(detail.instruments.map((instrument) => [String(instrument.id), instrument]));
  ...
  const equityInstruments = detail.instruments.filter((instrument) => EQUITY_KINDS.has(instrument.kind));
  const debtInstruments = detail.instruments.filter((instrument) => DEBT_KINDS.has(instrument.kind));
  const equityActivity = detail.payments.filter((payment) => {
    const instrument = instrumentsById.get(String(payment.instrumentId));
    return CAPITAL_KINDS.has(kindOf(payment)) && EQUITY_KINDS.has(instrument?.kind ?? "");
  });
  const rows = equityInstruments.map((instrument) => {
    const own = equityActivity.filter((payment) => String(payment.instrumentId) === String(instrument.id) && payment.currency === instrument.currency);
  ...
  const debtRows = detail.debt
    .map((debt) => ({ debt, instrument: instrumentsById.get(String(debt.instrumentId)) }))
    .filter((row) => !row.instrument || DEBT_KINDS.has(row.instrument.kind));
  return <div className="investors-panel">
    <div className="investors-card"><div className="investors-card-header"><div><h3>Equity capital</h3><p className="investors-note">Net capital is contributed less return of capital. Distributions remain separate.</p></div></div><div className="investors-table-wrap"><table className="investors-table"><thead><tr><th>Instrument</th><th className="amount">Committed</th><th className="amount">Contributed</th><th className="amount">Verified</th><th className="amount">Returned</th><th className="amount">Distributed</th><th className="amount">Net capital</th></tr></thead><tbody>{rows.map((row) => <tr key={row.instrument.id}><td><div className="investors-stack"><strong>{row.instrument.name}</strong><small>{label(row.instrument.kind)}</small></div></td><td className="amount">{money(row.instrument.committedCents, row.instrument.currency)}</td><td className="amount">{money(row.contributed.toString(), row.instrument.currency)}</td><td className="amount">{money(row.contributedVerified.toString(), row.instrument.currency)}</td><td className="amount">{money(row.returned.toString(), row.instrument.currency)}</td><td className="amount">{money(row.distributed.toString(), row.instrument.currency)}</td><td className="amount">{money((row.contributed - row.returned).toString(), row.instrument.currency)}</td></tr>)}</tbody><tfoot><tr><th scope="row">Shown: {rows.length} instruments</th><td className="amount">{moneyTotals(rows.map(row => ({ cents: row.instrument.committedCents, currency: row.instrument.currency })))}</td><td className="amount">{moneyTotals(rows.map(row => ({ cents: row.contributed.toString(), currency: row.instrument.currency })))}</td><td className="amount">{moneyTotals(rows.map(row => ({ cents: row.contributedVerified.toString(), currency: row.instrument.currency })))}</td><td className="amount">{moneyTotals(rows.map(row => ({ cents: row.returned.toString(), currency: row.instrument.currency })))}</td><td className="amount">{moneyTotals(rows.map(row => ({ cents: row.distributed.toString(), currency: row.instrument.currency })))}</td><td className="amount">{moneyTotals(rows.map(row => ({ cents: (row.contributed - row.returned).toString(), currency: row.instrument.currency })))}</td></tr></tfoot></table>{!rows.length && <div className="investors-empty"><h4>No equity instruments</h4></div>}</div></div>
    {(debtInstruments.length > 0 || debtRows.length > 0) && <div className="investors-card"><div className="investors-card-header"><div><h3>Debt principal</h3><p className="investors-note">Manual debt records show principal only; interest is excluded. The Debt tab carries schedule and rollforward detail when available.</p></div></div><div className="investors-table-wrap">{debtRows.length ? <table className="investors-table"><thead><tr><th>Instrument</th><th className="amount">Original principal</th><th className="amount">Funded capital</th><th className="amount">Manual outstanding principal</th><th>Maturity</th><th>Status</th></tr></thead><tbody>{debtRows.map(({ debt, instrument }) => <tr key={debt.id}><td>{instrument?.name ?? "Debt record"}</td><td className="amount">{money(debt.originalPrincipalCents, debt.currency)}</td><td className="amount">{money(debt.fundedCapitalCents, debt.currency)}</td><td className="amount">{money(debt.outstandingPrincipalCents, debt.currency)}</td><td>{dateLabel(debt.maturityOn)}</td><td>{instrument ? label(instrument.status) : "—"}</td></tr>)}</tbody><tfoot><tr><th scope="row">Shown: {debtRows.length} debt records</th><td className="amount">{moneyTotals(debtRows.map(({ debt }) => ({ cents: debt.originalPrincipalCents, currency: debt.currency })))}</td><td className="amount">{moneyTotals(debtRows.map(({ debt }) => ({ cents: debt.fundedCapitalCents, currency: debt.currency })))}</td><td className="amount">{moneyTotals(debtRows.map(({ debt }) => ({ cents: debt.outstandingPrincipalCents, currency: debt.currency })))}</td><td colSpan={2}>Principal only</td></tr></tfoot></table> : <div className="investors-empty"><h4>No debt balance record loaded</h4><p>Add or refresh a debt record before treating principal as known.</p></div>}</div></div>}
    <div className="investors-card"><div className="investors-card-header"><div><h3>Equity activity</h3><p className="investors-note">Contributions, returns and distributions are shown with their recorded status.</p></div></div><div className="investors-table-wrap"><table className="investors-table"><thead><tr><th>Date</th><th>Instrument</th><th>Kind</th><th className="amount">Amount</th><th>Status</th></tr></thead><tbody>{equityActivity.map((payment) => <tr key={payment.id}><td>{dateLabel(payment.paymentOn)}</td><td>{instrumentsById.get(String(payment.instrumentId))?.name ?? "—"}</td><td>{payment.reversesPaymentId ? `Reversal · ${label(kindOf(payment))}` : label(payment.kind)}</td><td className="amount">{money(payment.amountCents, payment.currency)}</td><td><Badge value={payment.status} /></td></tr>)}</tbody><tfoot><tr><th scope="row">Shown: {equityActivity.length} equity records</th><td>Counts only</td><td className="amount" colSpan={2}>Debt principal and interest remain in the Debt tab.</td><td /></tr></tfoot></table>{!equityActivity.length && <div className="investors-empty"><h4>No equity activity</h4></div>}</div></div>
  </div>;
}
  ...
  return <>
    ...
    <div className="investors-card" aria-label="Debt schedule"><div className="investors-card-header"><h3>Schedule and balance</h3>{debtInstruments.length > 1 && <select aria-label="Debt instrument" value={selected} onChange={(event) => setSelected(event.currentTarget.value)}>{debtInstruments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>}</div>
      {financialsError ? <Failure message={financialsError} onRetry={() => setAttempt((value) => value + 1)} /> : !financials ? <Loading text="Loading schedule…" /> : <>
        ...
        {amortization && amortization.rows.length > 0 && <details className="investors-form-section" open><summary>Debt service{amortization.levelPaymentCents ? ` · level payment ${money(amortization.levelPaymentCents, currency)}` : ""}</summary><div className="investors-table-wrap"><table className="investors-table"><thead><tr><th>Due</th><th>Phase</th><th className="amount">Opening</th><th className="amount">Interest</th><th className="amount">Principal</th><th className="amount">Balloon</th><th className="amount">Payment</th><th className="amount">Closing</th></tr></thead><tbody>{amortization.rows.map((row) => <tr key={row.periodMonth}><td>{dateLabel(row.dueOn)}</td><td>{label(row.phase)}</td><td className="amount">{money(row.openingCents, currency)}</td><td className="amount">{money(row.interestCents, currency)}</td><td className="amount">{money(row.principalCents, currency)}</td><td className="amount">{row.balloonCents === "0" ? "—" : money(row.balloonCents, currency)}</td><td className="amount">{money(row.paymentCents, currency)}</td><td className="amount">{money(row.closingCents, currency)}</td></tr>)}</tbody><tfoot><tr><th scope="row">Shown: {amortization.rows.length} schedule rows</th><td>Period totals</td><td className="amount">—</td><td className="amount">{moneyTotals(amortization.rows.map(row => ({ cents: row.interestCents, currency })))}</td><td className="amount">{moneyTotals(amortization.rows.map(row => ({ cents: row.principalCents, currency })))}</td><td className="amount">{moneyTotals(amortization.rows.map(row => ({ cents: row.balloonCents, currency })))}</td><td className="amount">{moneyTotals(amortization.rows.map(row => ({ cents: row.paymentCents, currency })))}</td><td className="amount">—</td></tr></tfoot></table></div></details>}
        {rollforward && rollforward.rows.length > 0 && <details className="investors-form-section"><summary>Monthly rollforward</summary><div className="investors-table-wrap"><table className="investors-table"><thead><tr><th>Month</th><th className="amount">Opening</th><th className="amount">Funded</th><th className="amount">Principal repaid</th><th className="amount">Interest paid</th><th className="amount">Unclassified</th><th className="amount">Closing</th></tr></thead><tbody>{rollforward.rows.map((row) => <tr key={row.periodMonth}><td>{monthLabel(row.periodMonth)}</td><td className="amount">{money(row.openingCents, currency)}</td><td className="amount">{money(row.fundedCents, currency)}</td><td className="amount">{money(row.principalRepaidCents, currency)}</td><td className="amount">{money(row.interestPaidCents, currency)}</td><td className="amount">{money(row.unclassifiedCents, currency)}</td><td className="amount">{money(row.closingCents, currency)}</td></tr>)}</tbody><tfoot><tr><th scope="row">Shown: {rollforward.rows.length} months</th><td className="amount">—</td><td className="amount">{moneyTotals(rollforward.rows.map(row => ({ cents: row.fundedCents, currency })))}</td><td className="amount">{moneyTotals(rollforward.rows.map(row => ({ cents: row.principalRepaidCents, currency })))}</td><td className="amount">{moneyTotals(rollforward.rows.map(row => ({ cents: row.interestPaidCents, currency })))}</td><td className="amount">{moneyTotals(rollforward.rows.map(row => ({ cents: row.unclassifiedCents, currency })))}</td><td className="amount">—</td></tr></tfoot></table></div></details>}
      </>}
    </div>
  </>;
}
