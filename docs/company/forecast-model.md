...
1. **Opening position** (`server/forecasting/sources.ts`). Each item carries an as-of date and state (`sourced`, `partial`, `manual`, `unknown`). Unknown items are excluded and listed ("Opening position incomplete: …"); they are never zero.
   ...
   **Unknown opening cash.** When operating or restricted opening cash is unknown, cash balances are movements relative to that unknown amount. `summary.openingCashKnown` is false; lowest available cash, ending cash, weeks below the floor and each week's `belowReserveFloor` are `null`; the workspace labels balances "(relative)" and hides the reserve-floor line; reports omit opening and closing cash (movements only) and the MCP output carries a `cashNote`. Approval of such a snapshot requires `acknowledgeIncompleteOpening: true` with a reason, which is recorded on the scenario (`approvalNote`).

2. **Operating drivers.** Unit lease schedule: occupied units run to lease end (month-to-month if none), then either renew for the renewal term at `rent × (1 + growth)` or move out, stay vacant for make-ready plus vacancy days, and start a new lease at market rent (grown on each scenario anniversary). An existing tenancy's current-month rent was billed before the cutoff and sits in opening receivables, so its first forecast charge is the next 1st. Rent is charged on the 1st (prorated for partial months). Housing assistance is a portion of contract rent, never an addition. Tenant collections (`collectionsBps`) and bad debt (`badDebtBps`) apply to the tenant portion after concessions; the write-off is capped at what collections leave, so both never exceed the amount due; any remainder stays in receivables. Managed properties collect into PM-held funds and remit each month's collections, net of the PM fee, after the remittance lag. Deposits are received at a new lease, returned after move-out, and transferred to a buyer on sale. Recurring costs accrue to payables and are paid after their lag; escrowed costs are paid from restricted cash.
...
`journal_balanced`, `balance_sheet_balances`, `cash_rollforward` (weeks and months), `direct_equals_indirect`, `retained_earnings_rollforward`, `debt_rollforward` (journal loan balances equal the schedules), `deposits_are_liabilities`, `subsidy_not_duplicated`, `weekly_monthly_agree` (monthly closing cash equals the weekly schedule at every month end inside the weekly horizon), `labor_not_duplicated`, `modeled_proceeds_not_actual` (refinance funding, sale and draw proceeds are flagged modeled, nothing else is, and none is dated in the actual period). A snapshot with any failed check cannot be approved.
...
| `forecast.scenario.update` | Name, kind, start, horizons, reserve floor. An approved scenario returns to draft. Snapshots record a hash of the start, horizons, reserve floor and currency (`parametersSha256`); a snapshot whose hash differs from the scenario's is stale. |
| `forecast.scenario.archive` | Archive; history stays readable. |
| `forecast.scenario.approve` | Pin a snapshot of the current version, settings and model whose checks all passed. Unknown opening cash requires `acknowledgeIncompleteOpening` with a reason. |
...
`line` × `period` (`W:<Monday>` or `M:<YYYY-MM>`): `cash.opening|closing|available|restricted|inflows|outflows|net|category.<category>`, `is.revenue|operating_expenses|noi|net_income|<account>`, `bs.<account>|retained_earnings|cash_total|receivables_total|property_net|payables_total|equity_total` (the composites are the balance-sheet composition segments), `cf.operating|investing|financing|net|direct.<category>`, `debt.service`, `ops.scheduled_rent`. The response lists the dated events that make up the figure (balance lines: opening balance plus the period's movements), paged by cursor, and the assumption inputs they came from. Contributions always sum to the displayed figure (tested).

### Reporting port

`createForecastReportingReadPort(executorOrPort, { principal })` implements `ForecastReportingReadPort`. The company reporting runtime passes the request's freshly loaded principal; without a principal the port refuses. Readers need the same organization-level grant as the workspace (owner, admin, finance, read-only reviewer), and the report scope must be the whole company (forecast reports offer no entity or property scope). Reports read **only the scenario's approved snapshot**: `inputVersion` must be that snapshot's ID or its assumption version (`3` / `v3`) and the model version must match; a draft scenario or another snapshot is `report_unavailable` ("No approved forecast scenario."). The catalog probe reports missing data until an approved scenario with a snapshot exists. Rows follow the report period: the 13 weeks starting with the first week the period touches (fewer than 13 is unavailable), months overlapping the period, loans outstanding during it and capital events closing within it; the known opening items are the actual boundary (dated at the cutoff). Growth metrics are exact dollar text (occupancy as percent text); loans carry the rate as an annual fraction (e.g. `0.0650`); a loan's refinance balance is set only when a refinance's `payoffLoanIds` pays it off. Coverage is `complete` only when the opening position is complete and every check passed; evidence is `reproducible_snapshot`.
...
## Known simplifications (fcst-1.1.0)
