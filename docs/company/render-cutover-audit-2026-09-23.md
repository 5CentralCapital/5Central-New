...
| Staging manager sign-in | `RENT_OPS_ADMIN_OAUTH_ORIGIN=https://fivecentral-ops-staging-web.onrender.com` accepted only inside that Render service with QBO not in production. | `server/admin-oauth.test.ts`, `render.staging.yaml` |
