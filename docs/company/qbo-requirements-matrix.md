...
Accessed 2026-09-22 from official Intuit sources. R-Ops is a **private, single-company internal app**. It is not listed on the QuickBooks App Store.

Applicability codes:

- **ALL**: applies to any app that holds production keys, including R-Ops.
...
| **Auth code.** Do not render the auth code; redirect to a clean URL. | D:…/oauth-2.0; D:go-live/publish-app/security-requirements | REC (MKT) | The callback always returns 303 to `/ops?…` and sets `Referrer-Policy: no-referrer` (`http.ts`). Since 2026-09-23 a missing admin session and known OAuth failures also redirect, with `qboError=<code>` (`http.test.ts`, `qbo-connection-lifecycle.test.ts`). | None |
...
| **Duplicate protection.** Send `requestid` (≤50 characters, unique per realm) on writes. | D:learn/learn-basic-field-definitions | REC | Every create and update sends `requestid`. The reconciler derives it from the operation key. Readback decides confirmation (`write-reconciliation.test.ts`). | None |
...
| **Deletions** appear only through CDC (`status: Deleted`, 30-day lookback, 1,000 objects) or webhooks. | D:learn/…/change-data-capture | REC | A full replay flags objects that QBO no longer returns. Incremental coverage is reported as partial. | CDC is not implemented |
...
| Requirement | Source | App. | Implementation evidence | Gap |
| --- | --- | --- | --- | --- |
| **Signature.** HMAC-SHA256 of the raw body using the verifier token, base64 encoded, sent in `intuit-signature`. Each environment has its own token. | D:develop/webhooks/configure-webhooks | ALL if used | `webhook.ts` uses a constant-time comparison (`webhook.test.ts`) | **No webhook route is registered.** R-Ops does not subscribe to webhooks. |
| **Delivery.** Reply 200 within 3 s. Delivery is at-least-once and out of order. Recover missed events with CDC. | D:develop/webhooks/best-practices | ALL if used | Parser plus de-duplication ledger interface (`filterNewQuickBooksWebhookEvents`) | Needs a route, a ledger table and a queue before the webhooks are enabled |
...
| Requirement | Source | App. | Implementation evidence | Gap |
| --- | --- | --- | --- | --- |
| **Production keys** appear only after the App Assessment Questionnaire is approved. This applies to private apps too. App details needed: EULA, privacy policy, host domain, launch URL, disconnect URL and reconnect URL. | D:go-live/publish-app; D:get-started/get-client-id-and-client-secret | ALL | Pages are implemented and deployed on `https://5central.capital`: `/legal/eula`, `/legal/privacy`, `/quickbooks/disconnected`, `/ops?section=accounting` (verified 2026-09-23) | The portal app-detail URLs and redirect URI must be entered with the `5central.capital` host. `/healthz` is intercepted by the host (use `/readyz`). |
| **Security.** Encrypt tokens at rest (AES) and keep the key separate. Never log credentials. Use Secure/HttpOnly cookies, HTTPS and TLS 1.2+. | D:go-live/publish-app/security-requirements | MKT (REC for R-Ops) | AES-GCM token cipher bound to scope (`token-crypto.ts`), security headers (`security-headers.ts`), redacted errors | None known |
